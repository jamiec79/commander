...
if [ "$(id -u)" = "0" ]; then
    ...
    echo -e "\nUse $(green "bash SWAPBlockscommander.sh") as a REGULAR user instead"
    echo -e "Execute ONCE $(green "chmod +x SWAPBlockscommander.sh") followed by $(green "ENTER")"
    echo -e "and start it only by $(green "./swapblockscommander.sh") as regular user after\n"
    exit 1
fi
...
if [ $(systemd-detect-virt -c) != "none" ]; then
    ...
    echo "$(ired "                                                                                 ")"
    echo "$(ired "     Running SWAPBlocks Node on a Container based virtual system is not recommended!    ")"
...
function proc_vars {
        ...
        # Looking for swapblocks-node installations and performing actions
        SBXdir=`locate -b "\swapblocks-node"`

        # Getting the parent of the install path
        parent=`dirname $SBXdir 2>&1`
...
#   echo -e "$(green "Public Key:")\n$(yellow "$PUBKEY")\n"
    ...
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
        if [ -e $SBXdir/app.js ]; then
                echo -e "\n$(green "       ✔ SWAPBlocks Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      SWAPBlocks Node process is running with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $SBXdir")\n"
                else
                        echo -e "\n$(red "       ✘ No SWAPBlocks Node process is running")\n"
                fi
        else
                echo -e "\n$(red "       ✘ No SWAPBlocks Node installation is found")\n"
        fi
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
...
function ntpd {
        ...
        if [ $(systemd-detect-virt) == "lxc" ] || [ $(systemd-detect-virt) == "openvz" ]; then
            ...
            sleep 2
                                if ! sudo pgrep -x "ntpd" > /dev/null; then
                                        echo -e "NTP failed to start! It should be installed and running for SWAPBlocks.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
                                        exit 1
                                fi
...
# Logrotate for SWAPBlocks Node logs
function log_rotate {
    if [[ "$(uname)" == "Linux" ]]; then

        if [ ! -f /etc/logrotate.d/swapblocks-logrotate ]; then
            echo -e " Setting up Logrotate for SWAPBlocks node log files."
            sudo bash -c "cat << 'EOF' >> /etc/logrotate.d/swapblocks-logrotate
$SBXdir/logs/swapblocks.log {
        size=50M
        copytruncate
...
function git_upd_check {
    ...
    if [ -d "$SBXdir" ]; then
        ...
        if [ "$LOCAL" == "$REMOTE" ]; then
            echo -e "         $(igreen "    SWAPBlocks Node is Up-to-date    \n")"
            UP_TO_DATE=1
...
function purge_pgdb {
    ...
    echo -e "    $(ired "   running SWAPBlocks Node processes and will  ")"
...
if [ "$(ls -A $SNAPDIR)" ]; then
    if [[ $(expr `date +%s` - `stat -c %Y $SNAPDIR/current`) -gt 900 ]]; then
        ...
        ## Numeric checks
            ...
            pg_restore -O -j 8 -d swapblocks $SNAPDIR/${snapshots[$(($REPLY-1))]} 2>/dev/null
        ...
        if [[ $? -eq 0 ]]; then
                ...
                echo -e "$(yellow "\n   Restoring $SNAPDIR/current ... ")"
                                pg_restore -O -j 8 -d swapblocks $SNAPDIR/current 2>/dev/null
...
# Install SWAPBlocks Node
function inst_swapblocks {
#   proc_vars
    cd $HOME
...
function db_exists {
        ...
        if [[ ! $(sudo -u postgres psql swapblocks -c '\q' 2>&1) ]]; then
...
function secret {
    ...
    #Put check if SBXdir is empty, if it is stays only config.mainnet.json
...
one(){
    ...
    if [ -e $SBXdir/app.js ]; then
        clear
        asciiart
        echo -e "\n$(green "       ✔ SWAPBlocks Node is already installed!")\n"
        if [ "$node" != "" ] && [ "$node" != "0" ]; then
                    echo -e "$(green "A working instance of SWAPBlocks Node is found with:")"
                    echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                    echo -e "$(green "and Work Directory: $SBXdir")\n"
                fi
        pause
        ...
        clear
        asciiart
        echo -e "$(yellow "           Installing SWAPBlocks node....")"
        create_db
        inst_swapblocks
        clear
        asciiart
        echo -e "$(green "          ✔ SWAPBlocks node was installed")\n"
        sudo updatedb
        sleep 1
        proc_vars
        log_rotate
...
# Reinstall SWAPBlocks Node
two(){
    clear
    asciiart
    echo -e "$(ired "!!! This option will erase your DB and SWAPBlocks Node installation !!!")\n"
    ...
    if [ "$keys" == "Y" ]; then
        proc_vars
            if [ -e $SBXdir/app.js ]; then
                    clear
                    asciiart
                    echo -e "\n$(green " ✔ SWAPBlocks Node installation found in $SBXdir")\n"
                    if [ "$node" != "" ] && [ "$node" != "0" ]; then
                            echo -e "$(green "A working instance of SWAPBlocks Node is found with:")"
                            echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                echo -e "$(yellow "           Stopping SWAPBlocks node ...")\n"
                cd $SBXdir
            ...
            if [ -e $parent/config.mainnet.json ] ; then
                ...
                if [ "$keys" == "Y" ]; then
                    cp $SBXdir/config.mainnet.json $parent
                    cd $parent
                fi
            else
                cp $SBXdir/config.mainnet.json $parent
...
#               echo "Break1"; pause
                if [ "$keys" == "Y" ]; then
                    cp $parent/config.mainnet.json $SBXdir
...
three(){
    ...
    if [ "$UP_TO_DATE" -ne 1 ]; then
            if [ "$node" != "" ] && [ "$node" != "0" ]; then
                    ...
                    echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
                    echo -e "$(green "       Directory: $SBXdir")\n"
            echo -e "\n$(green "             Updating SWAPBlocks Node...")\n"
            update_swapblocks
                    echo -e "$(green "                Restarting...")"
                    forever restart $forever_process >&- 2>&-
                    echo -e "\n$(green "    ✔ SWAPBlocks Node was successfully restarted")\n"
                    pause
        else
                    echo -e "\n$(red "       ✘ SWAPBlocks Node process is not running")\n"
            ...
            echo -e "         $(igreen " SWAPBlocks Node is already Up-to-date \n")"
            sleep 2
    fi
...
five(){
    ...
    if [ "$keys" == "Y" ]; then
            if [ "$node" != "" ] && [ "$node" != "0" ]; then
            echo -e "\n$(green "       Instance of SWAPBlocks Node found with:")"
            echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "       Directory: $SBXdir")\n"
            echo -e "$(green "                Restarting...")"
                    forever restart $forever_process >&- 2>&-
...
start(){
        ...
        if [ -e $SBXdir/app.js ]; then
                ...
                echo -e "\n$(green "       ✔ SWAPBlocks Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green " A working instance of SWAPBlocks Node was found with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $SBXdir")\n"
        else
            echo -e "$(green "            Starting SWAPBlocks Node...")\n"
            ...
            sleep 1
            proc_vars
            echo -e "\n$(green "       SWAPBlocks Node started with:")"
            echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "   and Work Directory: $SBXdir")\n"
                fi
    else
...
status(){
        ...
        if [ -e $SBXdir/app.js ]; then
                ...
                echo -e "\n$(green "       ✔ SWAPBlocks Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      SWAPBlocks Node process is working with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $SBXdir")\n"
                else
                        echo -e "\n$(red "       ✘ No SWAPBlocks Node process is running")\n"
...
killit(){
        ...
        if [ -e $SBXdir/app.js ]; then
                ...
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        ...
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $SBXdir")\n"
            echo -e "$(green "            Stopping SWAPBlocks Node...")\n"
            cd $SBXdir
            forever stop $forever_process >&- 2>&-
            cd $parent
            echo -e "$(green "    ✔ SWAPBlocks Node was successfully stopped")\n"
                else
            echo -e "\n$(red "       ✘ No SWAPBlocks Node process is running")\n"
                fi
        else
...
log(){
    ...
    proc_vars
    trap : INT
    tail -f $SBXdir/logs/swapblocks.log
#pause
}
...
show_menus() {
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "              1. Install SWAPBlocks"
    echo "              2. Reinstall SWAPBlocks"
    echo "              3. Update SWAPBlocks"
...
if [ -e ./.firstrun ] && [ "$fdate" =  "$sdate" ]; then
    ...
    if [ ! -e ./.firstrun ] ; then
        ...
        echo -e "\n$(ired "    !!!  PLEASE REBOOT YOUR SYSTEM NOW  !!!    ") "
          ...
          echo -e "$(ired "    !!!  CHOOSE '1' TO INSTALL SWAPBlocks NODE !!!    ") "
        exit
    fi
fi
